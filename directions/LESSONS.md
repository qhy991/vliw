...
| S11 | **tail-gap root cause @ 1093** (2026-07-06) | key_idx {0,2,3}=1093 (1,4 regress); mtmp=3 optimal (2→1116, ≥4 OOM); step no-op (fixed-shape uses `_POS_OFFSET_PSPACE`); combine_tail no-op (pspace uses explicit set). offset extremes: all-0(sync)=**1470**, 0..31=**1188**, i//2=**1217** — shipped `_POS_OFFSET_PSPACE_32x16`=1093 ≫ any regular offset | **Deeper root than S9/S10:** drain's last ~15 bundles have valu/alu at V0–V4/A0 (measured) — the pipeline runs out of *parallelism*, not load work. The **hash 6-stage serial chain (~11-level critical path/round)** forces ~11 near-empty bundles when only 1–2 vectors remain in drain. tail gap 57.5 ≈ hash-CP-depth × drain-serial-rounds. **Resurrection:** only a shorter hash critical path (algebraically rigid, #19a/#02) or a load-floor mover can shrink it — offset/engine-mask cannot (they don't change CP depth). |
...
| V11 | `#40` flip-p single-op traverse (2026-07-06) | **algebra PASS, perf NO-GO**: `probe_flip_p.py` bit-exact 20×256; but `probe_valu_floor.py` deletes all traverse `-` (the valu flip-p saves) → floor 999.5 yet realized **1094→1112**. valu is sub-floor (load 1035.5 binds) so the cut is absorbed AND the ops were tail-pack filler. Stacking w/ cold-mask: `cold=11` unchanged=1094 vs cut=1112. Doc `40-w7-subkilo-plans.md §2` | load floor drops below valu **and** tail gap is filled by real gather work (both required) |
| V12 | `#40` const→flow at scale (方案 A, 2026-07-06) | **NO-GO all 4 paths**: (1) multi-seed useless — bottleneck is 1-slot flow *capacity* not RAW (setup bundles all `flow=1`); (2) const can't defer out of setup — read by `broadcast_const` (dep=True); (3) vload const-table circular — materializing an imm needs load-const or flow-add_imm, no 3rd path; (4) setup→body merge = **1164** (greedy stretches const→bcast→bake RAW chain). load engine 94.6% saturated (117 free slots, 13 in windup). Doc `40 §4-A` | never (structural: consts on broadcast critical path) |
| V13 | `#40` store→vload gather permute (方案 C/G, 2026-07-06) | **mechanism NO-GO**: gather cost is on the READ side (source `tree[15+p]` scattered in non-contiguous mem). store→vload only reorders data *already in registers*; collecting `mem[tree[15+p_i]]` still needs 8 scalar loads. ISA (`problem.py:271-286`): no gather/scatter, no cross-lane permute, `vload` contiguous-only. d5–d10 = 1536/1984 loads all stuck on scattered-tree collection | never (ISA structural: no permute instruction) |
| V14 | `#47` traverse `rem` engine mask (`%`→`&` valu→alu) @ 1085 (2026-07-08) | **NEW movable class the annealer never had; still NO-GO.** valu binds 1056.3, F=1028, alu idle 914.7 — 448 `%`(==`&1`) are a free valu/alu choice, F-invariant. But prefix→alu monotonically regresses (N=32→**1111**, 448→1251); drain-tail→alu is only **cycle-neutral** (TAIL≤32=1085, 64→1094). Root cause: realized limited by drain critical path (S11), not valu floor; and `rem` is a **direct RAW producer** of the traverse `p←2p+rem`, so alu(8-slot) lengthens the CP (same shape as A2 vaddr→flow). Bit-exact (parity 0, submission OK). Default-off knobs `REM_ALU_N`/`REM_ALU_TAIL`/`REM_MASK`. Doc `47-rem-engine-mask-NOGO.md` | load-floor mover lands (drain gets gather work) → then co-anneal `rem` class with combine in omni_anneal (448 new drain knobs) |
...
| L7 | **valu-arith select for d4 cold-mux** (2026-07-06, S11 lever) | Replace flow-vselect with `muladd(c01, A−B, B)` where `c01=(0<cond)` — 3 valu ops swap 1 flow op, to feed idle drain valu instead of the 1-slot flow wall. **Bit-exact** (100k probe + parity 0/4096) BUT enlarging cold-mask regresses either way: rot29 `+{40}` valu-sel=1102 vs flow=1109 (both ≫ 1093); best of 10 single adds still +9. valu-sel beats flow only 2/10 and never beats baseline. | The shipped 11-instance cold-mask is already the **tail-packing optimum** (doc §3); adding ANY d4 instance breaks the schedule balance regardless of select engine. flow is NOT yet at its wall (859, ~140 slack) so paying 3× valu buys nothing. **Resurrection:** only if a load-floor mover creates genuine drain-valu slack AND flow actually saturates. |
