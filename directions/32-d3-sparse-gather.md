# Direction W6-W0 (E2 mirror): depth-3 sparse gather mask — **WIN 1134→1133**

**Status:** LANDED @ 1133 (2026-07-05). Branch `explore/w6-d3-sparse`.
Env `D3_GATHER_MASK` (JSON 0/1 over 64 d3 emit-order instances); shipped default
`{42}`; `D3_GATHER_MASK=[]` disables → 1134.
...
**Combo search** over {42,44,50,41,43,55,57} × sizes {1,2,3}: no combination beats
1133. `(42,55)`, `(44,55)` also reach 1133 but with a *higher* load floor (1051.5)
— pos 42 alone is the champion (floor 1047.5). Stacking gathers is additive on the
binding engine; the single drain hole is the whole prize.

## Composition with E2
Verified with the shipped `D4_COLD_MASK={25,26,27,29,31,34}` active: d3-pos-42 and
the d4 sparse mask compose (different rounds/engines) → 1133. Both defaults ship.

## Mechanism detail
- d3 rounds = {3, 14}; 64 d3 instances (2 rounds × K=32), emit-order interleaved by
  the 8-wide diagonal stagger. Pos 42 is in the round-14 (drain) half.
- Gather path needs the `fvp_p_3` broadcast (was gated behind `D3_GATHER_TAIL>0`);
  ...
  +1 setup const is live (funds the pos-42 gather).

## Gate (all pass)
`parity_check.py` 0 violations · `algebra_check_ported.py` ALL-PASS ·
`tests/submission_tests.py` **1133** · `PSPACE=0` **1187** (unchanged).

## Remaining headroom
The load floor is now 1047.5 with realized 1133 (gap 85.5). Adding more d3 gathers
only raises the floor. This win does not unlock #27 (load 1047.5 > alu 1036.7, gap
widened). The next load-floor *drop* still requires a scratch-free d4 cut (#26
gated) — d3 gather is a packing lever, not a floor lever.
